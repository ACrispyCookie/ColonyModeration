...
public class DiscordMute {
	...
	public static final ArrayList<DiscordMute> mutes = new ArrayList<>();
	...
	public void execute() {
		Main.getGuild().getTextChannelById(722982663526416454L).sendMessageEmbeds(DiscordMethods.getNormalMessage("Mute for the user " + target.getAsTag(), "React to this message with a ❌ to unmute the user!"
	...
	public void revoke(boolean wasFalse) {
		...
		if(wasFalse) {
			...
			Main.getGuild().getTextChannelById(722982663526416454L).retrieveMessageById(messageId).complete().editMessageEmbeds(DiscordMethods.getNormalMessage("User Unmuted",
					"The user **" + target.getAsTag() + "** has been unmuted!")).queue((msg) -> msg.delete().queueAfter(5, TimeUnit.SECONDS));
		}
		else {
