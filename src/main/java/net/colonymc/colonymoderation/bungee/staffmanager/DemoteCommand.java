				...
				if(args.length == 1) {
					if(!MainDatabase.getUuid(args[0]).equals("Not Found")) {
						...
						if(StaffMember.getByUuid(uuid) != null) {
							SpigotConnector.openActionMenu(p.getServer().getInfo(), p.getName(), uuid, StaffAction.DEMOTE.name());
						}
						else {
					...
					p.sendMessage(new TextComponent(ChatColor.translateAlternateColorCodes('&', " &5&l» &fUsage: &d/demote <player>")));
				}
			}
			else {
