		...
		try {
			...
			if(!MainDatabase.isConnecting()) {
				if(!MainDatabase.isConnected()) {
					e.setCancelReason(new TextComponent(ChatColor.translateAlternateColorCodes('&', "&5&l» &cAn error has occured while trying to connect the databases.\n&5&l» &cThe network will automatically restart when the database is back up!")));
					e.setCancelled(true);
				}
			}
			else {
				e.setCancelReason(new TextComponent(ChatColor.translateAlternateColorCodes('&', " &5&l» &cThe proxy is still starting up. Please wait!")));
				e.setCancelled(true);
			}
